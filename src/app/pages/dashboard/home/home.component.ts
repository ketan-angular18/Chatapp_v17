...
*/
  ...
  /**
   * clearChat for delete all messages from user
   * this function call dialog from dialog service and if user click on yes
   * then call deleteallmessage method from chatserivce that pass chatroomkey and delete all messages
   */
  async clearChat(): Promise<void> {
    const userClickDialog = await this.dialog.openYesNoDialog('Are You Sure Want To Delete All Messages')
    if (userClickDialog) {
      this.chatservice.deleteAllMessage(this.chatRoomKey).then(() => {
        this.toast.success('Chat is cleared')
      })
    }
  }
}
