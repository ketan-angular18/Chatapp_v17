...
export class ChatService {
  ...
  /**
   * deleteAllMessage for delete all message between the users
   * @param chatRoomid for pass refrence of chatroomid that you want to delete
   * @returns promise if error reject 
   */
  deleteAllMessage(chatRoomid:string) :Promise<void>{
    return this.db.database.ref(`chatRooms/${chatRoomid}/messages`)
    .remove().then(()=>{
      return Promise.resolve();
    })
    .catch((error)=>{
      console.error(`Error message: ${error.message}`);
        this.toast.error(
          'An error occurred during deleting chatroom'
        );
        return Promise.reject(error);
    })
  }
